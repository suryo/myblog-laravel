...
@section('content')
    ...
    <form action="{{ route('kelasonlinedetail.store') }}" method="POST">
                    ...
                    <div class="card-body">
                        <label class="form-label">Single Select</label>
                                    <select class="form-control select2" name="category_id">
                                        @foreach ($kelas_online_models as $kelas_online )
                                        <option value={{ $kelas_online->id }}>{{ $kelas_online->title }}</option>
                                        @endforeach
                                    </select>
                    </div>
                </div>
